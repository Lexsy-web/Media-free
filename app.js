(() => {
  ...
  if (isIndex) {
    ...
    const checkReady = (code, personalId, displayName) => {
      ...
      const updateWait = () => {
        ...
        if (waiting.length >= 2) {
          ...
          setStatus('Waiting for another user to join your code. Share the code with a friend and open it on another browser, tab, or device to join.');
        }
      };
    ...
    setStatus('Ready to connect. Enter a code (or leave 1234) and your name. Share the code with another device to chat across devices.');
  }
